# -*- coding: utf-8 -*-
"""
Takes a 2-dimensional matrix of size [m, n] (data) and deconvolutes
"""
...
class ds_user_script(UserOperation):
    ...
    convSigma = FloatSettingsObject(minimum=0.0, maximum=100, default=0.02)
    ...
    def operation(self, DataOut, Meta):
        ...
        simpOffset = Meta['J. Simplisma Offset %']
        mcrNoise = Meta['K. MCR Noise']
        ...
        startWaveIndex = len(np.where(waveVector <= startWave))
        endWaveIndex = len(np.where(waveVector <= endWave))
        startTimeIndex = len(np.where(timeVector <= startTime))
        endTimeIndex = len(np.where(timeVector <= endTime + 1))
        ...
        if startConditions == 'Simplisma':
            ...
            spectraList = [userSpectra[i].matrix for i in userSpectra]
            startingSpectra = np.array(spectraList)
        ...
        if deconMethod == 'Alternating Least Squares (ALS)':
            spectra, chromatograms = vuv.als(vuvData, startingSpectra,
                                             Meta, numIter=numIter,
                                             convSigma=convSigma)
        ...
        if initialMatches:
            for i, spectrum in enumerate(initialMatches):
                ...
                outInit.matrix = yVal
                outInit.name = 'InitMat {}: {}'.format(i, result)
                DataOut.append(outInit)
                outR2 = ScriptIOData()
                outR2.matrix = r2
                outR2.name = 'R2 {}: {}'.format(i, round(r2, 4))
                DataOut.append(outR2)
        if library:
            for i, spectrum in enumerate(spectra):
                result, r2, data = vuv.searchspectra(spectrum, library)
                yVal = vuv.interpolatespectra(data[0], data[1], waveVector)
                outLib = ScriptIOData()
                outLib.matrix = yVal
                outLib.name = 'Match {}: {}'.format(i, result)
